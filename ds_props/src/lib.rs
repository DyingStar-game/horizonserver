...
pub struct DyingstarPropsPlugin {
    ...
    planets: Arc<RwLock<HashMap<String, String>>>,
...
impl SimplePlugin for DyingstarPropsPlugin {
    ...
    async fn register_handlers(&mut self, events: Arc<EventSystem>, context: Arc<dyn ServerContext>) -> Result<(), PluginError> {
        ...
        events.on_plugin("propsplugin", "new_player", move |event: NewPlayerData| {
            ...
            runtime.spawn(async move {
                ...
                let sandbox_uuid = {
                    let planets = planets.read().await;
                    match planets.get("SandBox") {
                        Some(uuid) => uuid.clone(),
                        None => {
                            error!("🔧 DyingstarPropsPlugin: ❌ 'SandBox' planet not found in planets map!");
                            "".to_string()
                        }
                    }
                };
                let planet51_uuid = {
                    let planets = planets.read().await;
                    match planets.get("P5_M1") {
                        Some(uuid) => uuid.clone(),
                        None => {
                            error!("🔧 DyingstarPropsPlugin: ❌ 'P5_M1' planet not found in planets map!");
                            "".to_string()
                        }
                    }
                };
                let planet3_uuid = {
                    let planets = planets.read().await;
                    match planets.get("Gaea") {
                        Some(uuid) => uuid.clone(),
                        None => {
                            error!("🔧 DyingstarPropsPlugin: ❌ 'Gaea' planet not found in planets map!");
                            "".to_string()
                        }
                    }
                };
                ...
                let (base_x, base_y, base_z, parent_uuid) = match spawn_point {
                    ...
                    _ => (-2422100.0, 100.0, 0.0, sandbox_uuid),
                };
                ...
                if let Err(e) = events
                    .emit_plugin("genericprops", "create_object", &serde_json::json!({
                        "object_type": "player",
        ...
        events.on_plugin("props", "planet", move |event: serde_json::Value| {
            info!("props plugin received planet event: {:?}", event);
            let planets = planets_clone.clone();
            // Spawn an async task to handle the event
            runtime.spawn(async move {
                let planet_name = event["object_data"]["name"].as_str().unwrap_or("");
                let planet_uuid = event["object_uuid"].as_str().unwrap_or("");
                if !planet_name.is_empty() && !planet_uuid.is_empty() {
                    planets.write().await.insert(planet_name.to_string(), planet_uuid.to_string());
                    info!("🔧 DyingstarPropsPlugin: ✅ Registered planet '{}' with UUID {}", planet_name, planet_uuid);
                } else {
                    error!("🔧 DyingstarPropsPlugin: ❌ Invalid planet data received: {:?}", event);
                }
            });
            Ok(())
        }).await.unwrap();
    ...
    async fn on_init(&mut self, context: Arc<dyn ServerContext>) -> Result<(), PluginError> {
        let config = ds_common::config::Config::new("ds_props");
